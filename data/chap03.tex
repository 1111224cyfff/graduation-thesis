...
本章以“融合点云驱动的定位建图与动态目标候选生成”为核心，系统阐述双雷达点云实时融合、基于 LIO-SAM 的位姿估计与静态地图构建，以及基于 M-detector 的动态点检测、候选聚类与短时跟踪方法。整体处理流程遵循“先融合定位，后动态检测”的串行架构：前半部分聚焦于多源传感器数据的时序一致性、扫描结构适配与位姿连续性；后半部分则在统一世界坐标系下完成动态点提取、候选目标组织与短时状态输出。其中，定位模块旨在解决“平台自身位姿的鲁棒估计”问题，而动态检测模块则致力于将“点级动态事件”提升为“可跟踪目标对象”。

\begin{figure}[htbp]
	\centering
	\fbox{\parbox[c][5.6cm][c]{0.9\textwidth}{\centering
	占位图：第3章方法总流程图\\[0.5em]
	建议内容：双 Livox Mid-360 原始点云与a IMU 输入 $\rightarrow$ 静态外参加载、时间窗口同步与自动调平 $\rightarrow$ LIO-SAM 的 \texttt{imageProjection/featureExtraction/mapOptimization} 链路 $\rightarrow$ 输出连续位姿、静态地图与已配准点云 $\rightarrow$ M-detector 构建短时历史深度图并执行 Case1/2/3 动态判别 $\rightarrow$ 点云桥接、欧式聚类与短时跟踪状态输出。}}
