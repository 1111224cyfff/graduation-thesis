...
\subsection{1.2 研究目的与意义}\label{ux7814ux7a76ux76eeux7684ux4e0eux610fux4e49}
...
\subsubsection{\texorpdfstring{1.2.2 研究意义 }{1.2.2 研究意义 }}\label{ux7814ux7a76ux610fux4e49}
...
\subsection{1.3 国内外研究现状}\label{ux56fdux5185ux5916ux7814ux7a76ux73b0ux72b6}
...
\subsubsection{1.3.1 吊装感知硬件布置方案研究}\label{ux540aux88c5ux611fux77e5ux786cux4ef6ux5e03ux7f6eux65b9ux6848ux7814ux7a76}

\hl{突出：吊钩端研究匮乏，研究方法可延伸}

吊装作业是施工现场最常见的高危作业之一。为防范作业过程中可能对人员或建筑构件造成的损害，众多学者致力于通过智能感知技术与传感器部署来提升作业安全性。例如，在塔吊吊臂、小车、驾驶室及施工现场周边等位置，布设摄像机\textsuperscript{{[}7{]}}、惯性测量单元（Inertial Measurement Unit，IMU）\textsuperscript{{[}8,9{]}}、射频识别（RFID）\textsuperscript{{[}10{]}}、超宽带（Ultra Wide Band，UWB）\textsuperscript{{[}6{]}}、超声波传感器\textsuperscript{{[}11{]}}、高度传感器\textsuperscript{{[}12{]}}、风速传感器\textsuperscript{{[}13{]}}等多种传感器，以实时监测作业状态、识别周边障碍物与人员，从而实现风险预警。
...
针对塔吊间碰撞预警，Hwang 等\textsuperscript{{[}6{]}}在实验室环境下搭建了两个缩尺塔吊模型，在每台塔吊吊臂前端和后端各布置1个UWB 标签，并在实验场地角点固定布设 UWB 接收器，通过AoA与TDoA相结合的方式获取标签的空间位置信息。基于塔吊轴向旋转的运动特性，该方法采用标签间距离作为等效几何约束，将塔吊结构之间的安全距离映射为标签距离阈值，从而实现塔吊间潜在碰撞的实时预警。但该方案的传感器仍仅布设于塔吊结构本体，未在吊钩或吊物上增设独立感知单元，因此无法覆盖吊物与环境障碍物之间的碰撞风险，感知范围存在明显局限。（监控塔吊群的塔臂碰撞）
...
Zhou等\textsuperscript{{[}17{]}}将GNSS接收机固定于塔吊小车的支撑框架顶部，实时获取各塔吊塔臂端部在统一坐标系下的空间位置信息，从而实现对多塔空间关系的持续监测。基于 GNSS 解算得到的平面坐标数据，系统将塔臂简化为几何线段，并通过计算不同塔吊塔臂之间的相对距离来评估潜在的碰撞风险，当距离小于预设安全阈值时触发相应的碰撞预警。该方法通过引入 GNSS 定位传感器提升了多塔作业环境下的空间感知能力，但其传感器布设仍主要集中于塔吊结构本体，防碰撞对象以塔臂之间的结构干涉为主，未对吊钩或吊装物体进行直接感知。

张知田等\textsuperscript{{[}18{]}}提出面向``塔吊---工人''空间交互的危险场景自动检测框架，通过\textbf{工人位置}与\textbf{吊钩（等效吊物）位置}的同步获取实现预警。硬件布设上，该方法在\textbf{塔吊驾驶室下方架设远视角摄像头}以捕捉作业面内工人分布，同时在\textbf{吊臂滑动小车下方安装垂直向下摄像头}以估计吊钩/吊物的水平摆动偏移，并在塔吊主体布设\textbf{角度与测距传感器}获取吊臂旋转/下压角及小车、吊钩关键距离参数，通过局域网络实现数据实时传输。基于多源数据融合，方法先由图像估计吊钩偏移，再结合角度/测距数据计算吊钩三维坐标，并将工人与吊钩位置映射至 BIM 模型；在判别层面，以吊钩垂直投影点为中心构建10 m 圆形半径的动态危险区域，当工人进入该区域即触发危险场景预警。
...
Lee 等\textsuperscript{{[}19{]}}在塔吊盲吊导航系统中，将吊钩块作为关键观测对象：在变幅臂端安装激光传感器，并在\textbf{吊钩块安装反射板}作为测距靶标，利用激光束反射测得臂端到吊钩块的实际垂直距离。考虑到吊钩块剧烈摆动会导致``错失反射板''而产生误测，系统进一步结合吊钩块运动速度与方向对异常数据进行滤除，同时以放绳长度编码器作为备份测量源，最终将多源位置与视频信息发送至\textbf{驾驶室内的导航服务器}并在 BIM 模型中实时显示吊物位置与周边环境关系，主要用于提升盲吊场景下的态势感知与导航辅助。

在此基础上，为更直接表征``吊钩端摆动/旋转''等动态风险，Fang 等\textsuperscript{{[}20{]}}提出将\textbf{无线 IMU 刚性安装在吊物或吊钩端载荷上}，由 IMU 测得吊物的欧拉旋转角，在已知吊绳长度并近似刚性的前提下，可将姿态角进一步转换为吊物在三维空间中的相对位置，从而实现对吊物摆动状态的在线监测，并与编码器等机构传感数据结合用于安全辅助与告警。

不过，相关综述也指出这类``端部直测''方案往往要求在吊物/吊钩块/吊钩上增加额外硬件，带来电池维护、现场粗放工况下的损伤风险，以及金属环境对无线链路的干扰等工程约束。

进一步地，Ku 等\textsuperscript{{[}19{]}}在机器人化塔吊系统中将``吊钩块''视为末端执行器，直接在\textbf{吊钩块安装 3D LiDAR}获取近场点云以感知周围结构与潜在障碍，并将点云处理流水线部署在\textbf{吊钩块上的工业计算机}中实时提取路径方向上的障碍高度等关键信息，处理结果再无线回传至驾驶室内主机用于决策与避障执行。

除此之外，由于摄像机轻便的特性，许多研究者也尝试将摄像机安置于吊钩端，对下方拍摄感知，实现如施工危险对象识别\textsuperscript{{[}21{]}}、工人与负载碰撞预警\textsuperscript{{[}22{]}}等塔吊安全监测与预防研究。

\subsubsection{1.3.2 施工现场环境感知方法研究}\label{ux65bdux5de5ux73b0ux573aux73afux5883ux611fux77e5ux65b9ux6cd5ux7814ux7a76}
...
Yang 等\textsuperscript{{[}31{]}}采用 Mask R-CNN 对人员与障碍物进行语义分割，通过像素 - 实际距离转换模型，联合输出人员语义状态与安全距离，计算误差控制在 3\% 以内；Golcarenarenji 等\textsuperscript{{[}21{]}}设计的 CraneNet 深度学习模型，基于单目视觉实现 50 米范围内人员检测（语义识别）与远距离测量（几何距离），准确率达 92.59\%，且在嵌入式设备上实现 19 FPS 的实时性，适用于起重机驾驶室等空间受限场景。这类方案的优势在于部署成本低、语义信息提取能力强，但测距精度受图像分辨率与特征提取质量影响，弱光环境下性能易下降。双目视觉方案则通过立体匹配直接获取障碍物距离，几何精度高于单目，同时结合 CNN 实现人员 / 设备语义分类，但其测量范围受基线长度限制，近距离遮挡时匹配困难，设备体积也大于单目视觉，更适用于固定监测点的近距离作业场景。

视觉与其他传感器的融合方案进一步提升了联合感知的鲁棒性与精度。视觉与激光雷达融合方案中，视觉负责人员、设备、障碍物的语义识别，激光雷达提供高精度几何距离与三维点云，通过特征级融合将距离特征融入视觉检测网络，有效弥补视觉测距精度不足与激光雷达语义信息匮乏的缺陷，Price 等\textsuperscript{{[}32{]}}基于该方案实现盲吊场景的障碍物距离与人员语义联合预警，适配高精度要求的复杂环境；视觉 + RFID/IMU 融合方案则利用 RFID/IMU 的精准定位能力（几何距离）与视觉的行为语义提取优势，通过决策级融合实现人员动态风险预警，Zhang 等\textsuperscript{{[}33{]}}采用 FairMOT 算法跟踪人员与吊具，结合 Transformer 模型预测轨迹，联合人员位置信息与移动语义，建立多等级碰撞风险预警规则，提升人员密集场景的风险识别可靠性。此外，聚焦人员风险识别的专项融合方案成为研究焦点，Shapira 等\textsuperscript{{[}23{]}}的视觉系统通过人员检测与距离判断，在缩短起重机作业周期的同时降低安全风险。
...
数据级融合强调在原始数据层面实现信息联合，对多源测距数据进行坐标统一与点云拼接、对图像与深度/点云进行时空对齐后再进行联合建模。该策略的关键在于统一不同模态数据的表达尺度与坐标基准，从而在最底层最大化保留信息量，并为后续处理提供更一致的输入表示。Zhou和Omar\textsuperscript{{[}35{]}}采用像素级融合红外图像与RGB图像，采用自适应加权平均算法处理非饱和像素数据，针对饱和像素区域（眩光场景）实施主成分分析（PCA）算法，将两种输入的图像融合为单一输入，实现图像增强，以更鲁棒地检测实际场景中出现的人员和障碍。由于雷达和图像在水平与垂直分辨率上远低于光学图像\textsuperscript{{[}34{]}}，因此，需要在时空间上分别进行对齐，而非图像间的归一化像素级融合。Kim等\textsuperscript{{[}36{]}}综合使用 GPS、毫米波雷达、LiDAR 与视觉构建环境表示，通过多帧 LiDAR 观测累积生成网格地图，并以栅格``障碍观测计数''超过阈值触发风险预警，同时用雷达候选目标与视觉识别目标交叉验证并更新静态地图与安全区域。Lekic 与 Babic\textsuperscript{{[}37{]}} 利用 GAN 将雷达信息生成环境图像并与光学图像融合，Ouyang 等\textsuperscript{{[}38{]}}则使用条件 GAN 在图像监督下由点云重建语义场景图像，旨在缓解点云与相机直接融合带来的计算负担，并在 KITTI 数据集上验证实时检测有效性。

特征级融合通常先从各传感器数据中提取如几何特征、运动特征或深度网络特征的中间表示，再在特征空间完成联合建模，能够在信息利用与系统复杂度之间取得一定平衡，尤其在学习型感知框架中较为常见。传统机器学习时期往往需要先提取 HOG、GLCM 等人工特征，再用 SVM/AdaBoost 等分类器完成识别；而深度学习兴起后，特征融合更多被嵌入神经网络结构中，以端到端方式自动学习跨模态互补表示\textsuperscript{{[}34{]}}。在雷视融合任务中，特征级融合常用于道路/可行驶区域检测等场景，例如Caltagirone 等\textsuperscript{{[}39{]}}基于全卷积网络实现 LiDAR 与相机融合的道路检测。

更贴近工程避障预警需求的是目标属性级融合，各传感器先输出目标候选，再进行融合以降低单一传感器误报与漏报，该层级的抽象程度介于数据级融合与特征级融合之间，能够在保持计算可控的同时显著提升系统稳定性。一个与``吊钩端避障可视化''高度一致的范式是``LiDAR 生成 ROI → 图像中识别/解释 ROI''，如Wu等\textsuperscript{{[}40{]}} 利用 LiDAR 的距离与角度信息在图像中生成 ROI，并结合 3D 点云形状信息进一步验证目标以降低误报、缓解遮挡带来的相机漏检。同时，该方法通过匹配 LiDAR 与图像端的目标列表来提升检测速度并获得较高的行人检测精度。Han 等\textsuperscript{{[}41{]}}从系统实现角度强调实时性与工程落地，为``点云候选 + 视觉解释''的在线管线提供了工程参考。类似地，Zhong 等\textsuperscript{{[}42{]}}利用毫米波雷达运动信息提供图像 ROI，并在 ROI 上用 CNN 识别目标，同时合并雷达与相机的目标列表以提升鲁棒性。

决策级融合则由各传感器独立完成检测、识别或风险判定，再通过规则、置信度或逻辑推理进行融合输出，如采用贝叶斯推理、D-S 证据理论、模糊推理等。对工程避障而言，Wei 等\textsuperscript{{[}43{]}}直接面向工业场景的实时避碰系统，体现了``检测结果融合→避障决策''的应用导向。De Silva等\textsuperscript{{[}44{]}}则强调在移动机器人平台上实现鲁棒融合。
...
\subsubsection{1.3.3 碰撞检测算法与预警模型研究}\label{ux78b0ux649eux68c0ux6d4bux7b97ux6cd5ux4e0eux9884ux8b66ux6a21ux578bux7814ux7a76}

\hl{可以先写路径规划的避障策略，再写实际工程方面}

关于塔吊碰撞的检测与约束建模，既有研究首先在\textbf{塔吊吊装规划与虚拟仿真}场景中形成了较为系统的方法体系\textsuperscript{{[}45{]}}。在该场景下，研究人员多采用虚拟环境构建手段以模拟实际吊装场景，并以碰撞检测作为4D路径规划的约束条件\textsuperscript{{[}46{]}}，如通过为塔吊构建定向包围盒（Oriented bounding box，OBB）\textsuperscript{{[}47{]}}，采用离散碰撞检测（Discrete Collision Detection，DCD）\textsuperscript{{[}48,49{]}}或最小距离计算\textsuperscript{{[}50{]}}等方式判断碰撞是否发生。Lai和Kang\textsuperscript{{[}51{]}}中以简化外边界体（如球体、圆柱体等）近似现场机械与结构构件，从而显著降低实时碰撞检测的计算负担。这些方法在虚拟仿真环境下具有可行性，因为此时障碍物信息通常是已知且假设完全可观测的。但在实际场景中，周围环境的信息需要通过传感器实时获取，传感器的观测范围、精度以及数据延迟等问题，单纯依赖离线/静态几何模型的碰撞约束往往难以直接迁移。

针对施工过程中的实时安全辅助，研究逐渐从``是否碰撞''的二值判别，转向``\textbf{碰撞风险强度}''与``\textbf{分级预警}''建模。一类典型思路是构建安全包络或风险区域，采用最小距离/侵入判别触发告警\textsuperscript{{[}52{]}}。Wang等\textsuperscript{{[}53{]}}将风险区域与目标简化为AABB，并通过进入/逼近判别实现安全预警逻辑，体现了``几何包络 + 阈值规则''在实时系统中的工程可实现性。Yang等\textsuperscript{{[}60{]}} 使用Mask R-CNN检测危险源并根据照片与实际的像素转换关系换算计算工人与危险源的安全距离。
...
当障碍物与塔吊/吊载存在相对运动时，仅以``当前距离是否小于阈值''进行判断可能出现两类问题：其一，两个目标距离尚大但存在快速相向趋势，风险会在短时间内迅速上升；其二，阈值设置过大又会带来高误报，影响作业效率。因此，很多研究引入时间域 surrogate risk 指标，将风险刻画从``空间距离''推进到``时间余量''，典型如 Time-to-Collision（TTC）等。虽然 TTC 最初广泛用于交通安全 surrogate measures 的研究语境，Nadimi等\textsuperscript{{[}54{]}}对 TTC 等指标的适用性与局限进行了系统讨论，为``时间裕度型预警阈值如何设定与解释''提供了可借鉴的建模思路。在吊装场景下，将 TTC 类思想与相对速度/加速度估计结合，可形成``距离---速度联合预警''的框架，使预警更关注``是否会在未来短时窗内进入危险区''，而非仅关注当前几何接触。

根据Kim等\textsuperscript{{[}61{]}}的研究，相较于预防邻近的事故，预测潜在的碰撞事故能够更为有效地避免危险。。围绕预测驱动的碰撞预警，研究通常先对作业对象进行短时轨迹预测，再对未来时窗内的碰撞可能性进行判别或分级。张冬\textsuperscript{{[}62{]}}将碰撞预警方式区分为两种形式：针对塔吊与静止建筑物的碰撞，考虑为基于距离的预警；而针对塔吊与塔吊之间的碰撞，在距离模型的基础上结合速度模型来进行综合判断。进一步地，轨迹预测方法本身也形成了从统计模型到深度学习模型的谱系，例如通过隐马尔可夫模型（Hidden Markov Model，HMM）\textsuperscript{{[}63{]}}、长短期记忆人工神经网络（Long Short-Term Memory，LSTM）\textsuperscript{{[}64{]}}、深度神经网络 (Deep Neural Network，DNN)\textsuperscript{{[}65{]}} 等对人员或障碍物运动进行建模与预测。在获得塔吊与障碍物的未来轨迹后，可通过综合分析双方的相对运动趋势来进行风险判断，例如采用基于碰撞锥\textsuperscript{{[}66,67{]}}、基于约束优化等方法\textsuperscript{{[}68{]}}。

除风险度量模型外，动态场景下的碰撞检测还面临``实时性---精度/漏检风险''的工程折中问题。为此，相关研究在算法实现上通常采用``分层剔除 + 精细检测''的策略。在吊装路径规划与动态环境中，Dutta等\textsuperscript{{[}48{]}}提出了近实时重规划模块，并在决策支持部分采用多层级 OBB 来进行风险相关的判别与触发，体现了``层次包围体 + 决策逻辑''在动态场景中的可扩展性。Zhu等\textsuperscript{{[}55{]}}强调为提高碰撞检测效率与精度，需要融合空间划分与层次包围体等策略，并讨论了连续碰撞检测（CCD）在吊装扫掠体场景下的重要性。同样地，Lin等\textsuperscript{{[}56{]}}提出基于点云的碰撞检测方法并将其嵌入规划流程，反映出``直接用现场环境状态驱动碰撞判别''的趋势。

在更一般的动态避碰理论中，碰撞风险还可通过``相对速度可达集合''进行刻画，例如碰撞锥（collision cone）与速度障碍（velocity obstacle, VO）范式。Fiorini and Shiller早在\textbf{1998年便}提出了 VO 框架以在动态环境中选择规避机动\textsuperscript{{[}57{]}}，其思想可用于将塔吊端执行体与动态障碍的相对运动关系显式化。Van den Berg等\textsuperscript{{[}58{]}}将该思想扩展到多主体实时避碰，为复杂多障碍、多作业体并行施工条件下的风险判别提供了更系统的理论工具。进一步地，安全关键建模常将避碰问题表述为``满足安全集约束''的控制问题，控制障碍函数（CBF）成为近年来重要方向之一。Jian等\textsuperscript{{[}59{]}}提出了D-CBF，将障碍物预测与安全约束结合，用以保证动态避障的可行性与安全性，这为``预警模型如何与后续控制/干预策略耦合''提供了可迁移的建模范式。
...
\subsection{1.4 主要研究内容}\label{ux4e3bux8981ux7814ux7a76ux5185ux5bb9}
...
\section{第2章 吊钩端传感器系统构建与配准}\label{ux7b2c2ux7ae0-ux540aux94a9ux7aefux4f20ux611fux5668ux7cfbux7edfux6784ux5efaux4e0eux914dux51c6}

\subsection{2.1 系统总体架构与硬件选型布置}\label{ux7cfbux7edfux603bux4f53ux67b6ux6784ux4e0eux786cux4ef6ux9009ux578bux5e03ux7f6e}

\subsubsection{2.1.1 传感设备选型}\label{ux4f20ux611fux8bbeux5907ux9009ux578b}

\hl{双 LiDAR + 工业相机，说明选型依据：视场覆盖、语义识别需求}

\subsubsection{2.1.2 安装布局优化}\label{ux5b89ux88c5ux5e03ux5c40ux4f18ux5316}

LiDAR 盲区互补、相机视场协同

\subsubsection{2.1.3 吊钩端一体化盒体设计}\label{ux540aux94a9ux7aefux4e00ux4f53ux5316ux76d2ux4f53ux8bbeux8ba1}

（工程可落地性：震动、粉尘、线缆、通信/计算/供电）

\subsection{2.2 多传感器外参标定}\label{ux591aux4f20ux611fux5668ux5916ux53c2ux6807ux5b9a}

\subsubsection{2.2.1 相机内参标定}\label{ux76f8ux673aux5185ux53c2ux6807ux5b9a}

\subsubsection{2.2.2 双雷达外参自动标定与精配准}\label{ux53ccux96f7ux8fbeux5916ux53c2ux81eaux52a8ux6807ux5b9aux4e0eux7cbeux914dux51c6}

（已完成：粗配准+自动精配准→变换矩阵）

\subsubsection{2.2.3 雷达--相机外参标定}\label{ux96f7ux8fbeux76f8ux673aux5916ux53c2ux6807ux5b9a}

（为第4章投影关联做铺垫）

\subsection{2.3数据链路与接口规范}\label{ux6570ux636eux94feux8defux4e0eux63a5ux53e3ux89c4ux8303}

\subsubsection{2.3.1 通信链路搭建}\label{ux901aux4fe1ux94feux8defux642dux5efa}

（双网口/桥接/转发逻辑、传输稳定性）

\subsubsection{2.3.2 时间同步与延迟补偿}\label{ux65f6ux95f4ux540cux6b65ux4e0eux5ef6ux8fdfux8865ux507f}

（LiDAR/相机/IMU，时序一致性保障）

\subsubsection{2.3.3 坐标系与 TF 接口规范}\label{ux5750ux6807ux7cfbux4e0e-tf-ux63a5ux53e3ux89c4ux8303}

（空间参考约定，不涉融合细节）

\section{第3章 基于融合点云的定位与空间危险区域检测方法}\label{ux7b2c3ux7ae0-ux57faux4e8eux878dux5408ux70b9ux4e91ux7684ux5b9aux4f4dux4e0eux7a7aux95f4ux5371ux9669ux533aux57dfux68c0ux6d4bux65b9ux6cd5}

还有另一个问题。对于扫描后点云的预处理工作是在lio-sam前进行还是lio-sam后进行？

\subsection{3.1 双雷达点云实时融合方法}\label{ux53ccux96f7ux8fbeux70b9ux4e91ux5b9eux65f6ux878dux5408ux65b9ux6cd5}

\subsubsection{3.1.1 原始点云数据特征}\label{ux539fux59cbux70b9ux4e91ux6570ux636eux7279ux5f81}

\subsubsection{3.1.2 外参矩阵应用与坐标统一}\label{ux5916ux53c2ux77e9ux9635ux5e94ux7528ux4e0eux5750ux6807ux7edfux4e00}
...
在完成\textbf{前两步旋转}后，沿\textbf{当前自身坐标系的} \(x\)\textbf{轴负方向}平移：169.204874mm（177.140239mm）
...
\subsection{3.2 基于融合点云的SLAM定位与静态地图构建（LIO-SAM）}\label{ux57faux4e8eux878dux5408ux70b9ux4e91ux7684slamux5b9aux4f4dux4e0eux9759ux6001ux5730ux56feux6784ux5efalio-sam}

\subsubsection{3.2.1 算法选择理由与系统输入输出}\label{ux7b97ux6cd5ux9009ux62e9ux7406ux7531ux4e0eux7cfbux7edfux8f93ux5165ux8f93ux51fa}

\subsubsection{3.2.2 LIO-SAM流程}\label{lio-samux6d41ux7a0b}

（前端里程计、后端因子图/回环等按实现程度写）

\subsubsection{3.2.3 位姿输出与点云去畸变/配准}\label{ux4f4dux59ffux8f93ux51faux4e0eux70b9ux4e91ux53bbux7578ux53d8ux914dux51c6}

（与后续危险检测的关系）

\subsection{3.3 面向目标提取的点云标准化与静动态目标生成}\label{ux9762ux5411ux76eeux6807ux63d0ux53d6ux7684ux70b9ux4e91ux6807ux51c6ux5316ux4e0eux9759ux52a8ux6001ux76eeux6807ux751fux6210}

这一节明确：只产出``目标''（静态障碍表征 + 动态簇候选），不做风险判定。

\subsubsection{3.3.1 近场关注区域ROI裁剪与点云预处理}\label{ux8fd1ux573aux5173ux6ce8ux533aux57dfroiux88c1ux526aux4e0eux70b9ux4e91ux9884ux5904ux7406}

（ROI裁剪、滤波、下采样、离群点抑制；强调实时性与鲁棒性）

\subsubsection{3.3.2 基于位姿的时空对齐与局部滑窗表示}\label{ux57faux4e8eux4f4dux59ffux7684ux65f6ux7a7aux5bf9ux9f50ux4e0eux5c40ux90e8ux6ed1ux7a97ux8868ux793a}

（利用 3.2 位姿做坐标统一；滑窗累积作为背景参考/时序容器）

\subsubsection{3.3.3 静动态点分离与动态点云簇提取}\label{ux9759ux52a8ux6001ux70b9ux5206ux79bbux4e0eux52a8ux6001ux70b9ux4e91ux7c07ux63d0ux53d6}

（时序一致性/地图一致性；聚类输出动态簇中心、尺寸、包围盒）

\subsubsection{3.3.4 目标短时跟踪与状态量估计}\label{ux76eeux6807ux77edux65f6ux8ddfux8e2aux4e0eux72b6ux6001ux91cfux4f30ux8ba1}

（卡尔曼/匀速模型；输出速度、轨迹，为趋势预测服务）

\section{第4章 基于雷视融合的避障预警策略}\label{ux7b2c4ux7ae0-ux57faux4e8eux96f7ux89c6ux878dux5408ux7684ux907fux969cux9884ux8b66ux7b56ux7565}

\subsection{4.1 视觉侧高风险目标识别}\label{ux89c6ux89c9ux4fa7ux9ad8ux98ceux9669ux76eeux6807ux8bc6ux522b}

\subsubsection{\texorpdfstring{4.1.1 工人检测模型与部署 }{4.1.1 工人检测模型与部署 }}\label{ux5de5ux4ebaux68c0ux6d4bux6a21ux578bux4e0eux90e8ux7f72}

（实时性、输入分辨率、输出格式）

\subsubsection{4.1.2 检测结果时序稳定化处理}\label{ux68c0ux6d4bux7ed3ux679cux65f6ux5e8fux7a33ux5b9aux5316ux5904ux7406}
...
视觉置信度 s\_det 做滑动平均：s̄\_det(t) = α s̄\_det(t-1) + (1-α) s\_det(t)

s̄\_det \textgreater{} Th\_on 才把目标升级为``可信工人''

s̄\_det \textless{} Th\_off 才降级/移除

\subsubsection{4.1.3 点云动态目标融合接口与数据结构定义}\label{ux70b9ux4e91ux52a8ux6001ux76eeux6807ux878dux5408ux63a5ux53e3ux4e0eux6570ux636eux7ed3ux6784ux5b9aux4e49}
...
\subsection{4.2 雷视融合策略}\label{ux96f7ux89c6ux878dux5408ux7b56ux7565}

（从``能投影''到``能关联''）双向一致性关联与轨迹级语义增强

\subsubsection{4.2.1 融合总体流程与设计原则}\label{ux878dux5408ux603bux4f53ux6d41ux7a0bux4e0eux8bbeux8ba1ux539fux5219}

（LiDAR主几何、视觉提供语义与验证）

\subsubsection{4.2.2 双相一致性关联策略}\label{ux53ccux76f8ux4e00ux81f4ux6027ux5173ux8054ux7b56ux7565}

2D→3D门控关联（从检测框到点云候选）+3D→2D反向验证（从动态簇到图像一致性确认）

\subsubsection{4.2.3 轨迹级绑定与遮挡鲁棒融合}\label{ux8f68ux8ff9ux7ea7ux7ed1ux5b9aux4e0eux906eux6321ux9c81ux68d2ux878dux5408}

（2D track ↔ 3D track）

\subsubsection{4.2.4 置信度融合与语义标签输出}\label{ux7f6eux4fe1ux5ea6ux878dux5408ux4e0eux8bedux4e49ux6807ux7b7eux8f93ux51fa}

（human / non-human / unknown + score）

Score:融合视觉置信度、点云动态性、点数密度、时序稳定性，形成一个置信度打分机制

\subsection{4.3 风险判定与分级预警策略}\label{ux98ceux9669ux5224ux5b9aux4e0eux5206ux7ea7ux9884ux8b66ux7b56ux7565}

距离阈值 + 运动趋势预测

\subsubsection{4.3.1 静态障碍：基于距离/侵入的阈值预警}\label{ux9759ux6001ux969cux788dux57faux4e8eux8dddux79bbux4fb5ux5165ux7684ux9608ux503cux9884ux8b66}

（用第3章的危险域判定结果，距离/侵入阈值、滞回）

\subsubsection{4.3.2 动态目标：基于相对速度/轨迹预测的碰撞风险}\label{ux52a8ux6001ux76eeux6807ux57faux4e8eux76f8ux5bf9ux901fux5ea6ux8f68ux8ff9ux9884ux6d4bux7684ux78b0ux649eux98ceux9669}

（TTC/最小距离时刻等）

\subsubsection{4.3.3 分级预警状态机}\label{ux5206ux7ea7ux9884ux8b66ux72b6ux6001ux673a}

（Normal/Warning/Danger，滞回与抑制误报）

\section{第5章 实验验证与系统评估}\label{ux7b2c5ux7ae0-ux5b9eux9a8cux9a8cux8bc1ux4e0eux7cfbux7edfux8bc4ux4f30}

\subsection{5.1 实验平台与测试场景设置}\label{ux5b9eux9a8cux5e73ux53f0ux4e0eux6d4bux8bd5ux573aux666fux8bbeux7f6e}

\subsubsection{5.1.1 吊钩端实验平台构成}\label{ux540aux94a9ux7aefux5b9eux9a8cux5e73ux53f0ux6784ux6210}

\subsubsection{5.1.2 典型测试工况设计}\label{ux5178ux578bux6d4bux8bd5ux5de5ux51b5ux8bbeux8ba1}

\subsection{5.2 多传感器定位性能实验}\label{ux591aux4f20ux611fux5668ux5b9aux4f4dux6027ux80fdux5b9eux9a8c}

\subsubsection{5.2.1 位姿解算精度验证}\label{ux4f4dux59ffux89e3ux7b97ux7cbeux5ea6ux9a8cux8bc1}

\subsubsection{5.2.2 吊钩动态摆动场景分析}\label{ux540aux94a9ux52a8ux6001ux6446ux52a8ux573aux666fux5206ux6790}

\subsection{5.3 雷视协同感知效果分析}\label{ux96f7ux89c6ux534fux540cux611fux77e5ux6548ux679cux5206ux6790}

\subsubsection{5.3.1 工人检测准确性评估}\label{ux5de5ux4ebaux68c0ux6d4bux51c6ux786eux6027ux8bc4ux4f30}

\subsubsection{5.3.2 雷视关联一致性分析}\label{ux96f7ux89c6ux5173ux8054ux4e00ux81f4ux6027ux5206ux6790}

\subsection{5.4 危险区域检测效果验证}\label{ux5371ux9669ux533aux57dfux68c0ux6d4bux6548ux679cux9a8cux8bc1}

\subsubsection{5.4.1 静态障碍场景测试}\label{ux9759ux6001ux969cux788dux573aux666fux6d4bux8bd5}

\subsubsection{5.4.2 动态人员接近场景测试}\label{ux52a8ux6001ux4ebaux5458ux63a5ux8fd1ux573aux666fux6d4bux8bd5}

\subsection{5.5 系统实时性与稳定性验证}\label{ux7cfbux7edfux5b9eux65f6ux6027ux4e0eux7a33ux5b9aux6027ux9a8cux8bc1}

\subsubsection{5.5.1 各模块耗时分析}\label{ux5404ux6a21ux5757ux8017ux65f6ux5206ux6790}

\subsubsection{5.5.2 工程应用可行性分析}\label{ux5de5ux7a0bux5e94ux7528ux53efux884cux6027ux5206ux6790}

\section{第6章 结论与展望}\label{ux7b2c6ux7ae0-ux7ed3ux8bbaux4e0eux5c55ux671b}

\subsection{6.1 主要研究结论}\label{ux4e3bux8981ux7814ux7a76ux7ed3ux8bba}

\subsection{6.2 创新点总结}\label{ux521bux65b0ux70b9ux603bux7ed3}

\subsection{6.3 存在的不足与未来展望}\label{ux5b58ux5728ux7684ux4e0dux8db3ux4e0eux672aux6765ux5c55ux671b}
...
{[}61{]} Kim D, Liu M, Lee S, et al. Trajectory Prediction of Mobile Construction Resources Toward Pro-active Struck-by Hazard Detection{[}C/OL{]}//36th International Symposium on Automation and Robotics in Construction. Banff, AB, Canada, 2019{[}2026-01-08{]}. http://www.iaarc.org/publications/2019\_proceedings\_of\_the\_36th\_isarc/trajectory\_prediction\_of\_mobile\_construction\_resources\_toward\_pro\_active\_struck\_by\_hazard\_detection.html.
...
{[}69{]} Vaswani A, Shazeer N, Parmar N, 等. Attention is All you Need{[}C/OL{]}//Advances in Neural Information Processing Systems: 卷 30. Curran Associates, Inc., 2017{[}2026-01-08{]}. https://proceedings.neurips.cc/paper\_files/paper/2017/hash/3f5ee243547dee91fbd053c1c4a845aa-Abstract.html.
